...
public class BitbucketService {
    ...
    @Cacheable(key = "#root.methodName + #pathAt")
    public Optional<Pair<MediaType, String>> getCachedTextFileContents(BitbucketPathAt pathAt)
            throws BitbucketInvalidEntityException {
        return this.getTextFileContents(pathAt);
    }

    /*
        WARNING: This method is not cached, for most of the cases, see #getCachedTextFileContents
     */
    public Optional<Pair<MediaType, String>> getTextFileContents(BitbucketPathAt pathAt)
            throws BitbucketInvalidEntityException {
        log.debug("No cache entry detected for pathAt: {}", pathAt);
        return this.getFileContents(pathAt.getProjectKey(), pathAt.getRepoSlug(), pathAt.getSubPath(),
                pathAt.getAt(), String::new);
    }

    @Cacheable(key = "#root.methodName + #pathAt")
    public Optional<Pair<MediaType, byte[]>> getCachedBinaryFileContents(BitbucketPathAt pathAt)
            throws BitbucketInvalidEntityException {
        return this.getFileContents(pathAt.getProjectKey(), pathAt.getRepoSlug(), pathAt.getSubPath(),
    ...
    @Cacheable(key = "#root.methodName + #pathAt")
    public boolean doesContributingFileExists(BitbucketPathAt pathAt) {
        try {
            ResponseEntity<Void> response = projectApi.streamContributingWithHttpInfo
                    (pathAt.getProjectKey(), pathAt.getRepoSlug(), pathAt.getAt(), null, null,
                            null, null);
    ...
    public Optional<String> getLastCommit(BitbucketPathAt pathAt) {
        ResponseEntity<GetCommits200Response> response = repositoryApi.getCommitsWithHttpInfo(
                pathAt.getProjectKey(),
                pathAt.getRepoSlug(),
                null, // avatarScheme
                pathAt.getSubPath(), // this filters commits by file path
                null, null, pathAt.getAt(), null, null, null, null,
                BigDecimal.valueOf(0), BigDecimal.valueOf(1) // get the latest commit only
        );
