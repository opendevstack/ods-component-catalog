...
import org.opendevstack.component_catalog.server.services.bitbucket.BitbucketPathAt.BitbucketPathAtBuilder;
...
import org.opendevstack.component_catalog.util.MarkdownUtils;
...
public class FileEntitiesService {
    ...
    public Optional<Pair<MediaType, byte[]>> getFileById(String id) throws InvalidIdException, InvalidEntityException {
        ...
        try {
            return bitbucketService.getCachedBinaryFileContents(filePathAt.get());
    ...
    private Supplier<Pair<String, byte[]>> imgSupplier(BitbucketPathAt imgPathAt) {
        return () -> {
            var imgContents = bitbucketService.getCachedBinaryFileContents(imgPathAt);
