...
public class PaginationUtils {
    ...
    public static Pagination buildPagination(int page, int size, int totalElements, String basePath) {
        ...
        var next = (page < totalPages - 1) ? URI.create(basePath + "?page=" + Math.max((page + 1), 0) + "&size=" + size) : null;
        var previous = (page > 0) ? URI.create(basePath + "?page=" + Math.min((page - 1), totalPages) + "&size=" + size) : null;
